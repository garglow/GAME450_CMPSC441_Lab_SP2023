...
#returns a dictionary of dictionaries of a list
...
def run_episodes(n_episodes):
    ...
    for i in range(0,n_episodes):
        #run run_random_episode with the 2 created players, returns list of tuples
        history = run_random_episode(player, opponent)
        #run get_history_returns with list of tuples generated by run_random_episode
        history_returns = get_history_returns(history)
        #access dictionary of dictionries generated by get_history_returns to find average action value
        for state in history_returns:
            if state not in action_values:
                action_values[state] = {}
            for action in history_returns[state]:
                if action not in action_values[state]:
                    action_values[state][action] = []
                action_values[state][action].append(history_returns[state][action])
