...
def get_route_cost(route_coordinate, game_map, elevation):
    """
    ...
    #get elevation for starting and end coord
    elevation1 = elevation[x1][y1]

    elevation2 = elevation[x2][y2]

    #find difference between them and add on factor of difference
    difference = abs(elevation1 - elevation2)
    
    totalCost = game_map[tuple(zip(*path))].sum() + (100 * difference)
...
def main():
    ...
    elevation = []
    elevation = get_elevation(map_size)
    ...
    #get travel cost now utilizes elevation
    for route, route_coordinate in zip(routes, route_coordinates):
        print(f'Cost between {route[0]} and {route[1]}: {get_route_cost(route_coordinate, game_map, elevation)}')
